"""
Copied from pybullet-gym, changed a little bit to meet PEP-8 style
"""
import json
import os
import subprocess
...
class VideoRecorder(object):
    ...
    def __init__(self, env, path=None, metadata=None, enabled=True, base_path=None):
        ...
        if actual_ext != required_ext:
            hint = " HINT: The environment is text-only, " \
                   "therefore we're recording its text output in a structured JSON format." if self.ansi_mode else ''
            raise error.Error(
    ...
    def capture_frame(self):
        ...
        if frame is None:
            if self._async:
                ...
                # Indicates a bug in the environment: don't want to raise an error here.
                logger.warn(
                    'Env returned None on render(). Disabling further rendering for video recorder '
                    'by marking as disabled: path=%s metadata_path=%s',
    ...
    def __init__(self, output_path, frame_shape, frames_per_sec):
        ...
        if pixfmt != 3 and pixfmt != 4:
            raise error.InvalidFrame(
                ...
                "RGB values for a w-by-h image, with an optional alpha channl.".format(
                    frame_shape))
        self.wh = (w, h)
        ...
        if distutils.spawn.find_executable('avconv') is not None:
            ...
            raise error.DependencyNotInstalled(
                ...
                you can install ffmpeg via `brew install ffmpeg`. On most Ubuntu variants, 
                `sudo apt-get install ffmpeg` should do it. On Ubuntu 14.04, however, you'll need 
                to install avconv with `sudo apt-get install libav-tools`.""")
